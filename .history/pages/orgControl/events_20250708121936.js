...
export default function EventsPage() {
  ...
  const fetchEvents = async () => {
    try {
      ...
      // If no organizations found, set empty events and return
      ...
      const adminOrgIds = organizations.map((org) => org._id);
      ...
      if (eventsResponse.data && eventsResponse.data.events) {
        ...
        const filteredEvents = eventsResponse.data.events.filter(
          (event) =>
            event.organiser && adminOrgIds.includes(event.organiser._id)
        );
        ...
        const transformedEvents = filteredEvents.map((event) => ({
          ...
          sponsors: safeArray(event.sponsors),
          agenda: safeString(event.agenda),
          organiser: event.organiser, // Keep the original organiser object
        }));
